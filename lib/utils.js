...
exports.createGetterSetter = function (collection, item, prop) {
  ...
  if (collection.hasOwnProperty(key) === false) {
    Object.defineProperty(collection, key, {
      enumerable: true,
      get: function () {
        return prop ? collection[prop].get(key) : collection.get(key);
      },
      set: function(value) {
        prop ? collection[prop].set(value) : collection.set(value);
      }
    });
  }
};
