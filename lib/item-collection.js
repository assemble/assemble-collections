...
ItemCollection.prototype.get = function (search, by) {
  ...
  if (typeof search === 'string') {
    if (by && typeof by === 'string') {
      return this.findByContext(search, by);
    }
...
ItemCollection.prototype.sorted = function(compare, by, order) {
  ...
  if (_.isString(compare)) {
    ...
    compare = function (a, b) {
      if (typeof a.context[search] !== 'undefined' && typeof b.context[search] !== 'undefined') {
        if (a.context[search] > b.context[search]) {
          return 1;
        } else if (a.context[search] < b.context[search]) {
          return -1;
        }
      }
      return 0;
...
ItemCollection.prototype.findByContext = function (search, property) {
  var results = this.items.find(search, function (obj, search) {
    var value = traverse(obj.context).get(property.split('.'));
    return value === search;
  });
