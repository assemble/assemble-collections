...
ItemCollection.prototype.sorted = function(compare, by, order) {
  ...
  if (_.isString(compare)) {
    ...
    compare = function (a, b) {
      if (typeof a.metadata[search] !== 'undefined' && typeof b.metadata[search] !== 'undefined') {
        if (a.metadata[search] > b.metadata[search]) {
          return 1;
        } else if (a.metadata[search] < b.metadata[search]) {
          return -1;
        }
      }
      return 0;
...
ItemCollection.prototype.findByMetadata = function (search, property) {
  var results = this.items.find(search, function (obj, search) {
    var value = traverse(obj.metadata).get(property.split('.'));
    return value === search;
  });
